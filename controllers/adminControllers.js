...
exports.postAddProduct = async (req, res, next) => {
  const { initialValues, product, images } = req.body;
  ...
  if (!initialValues) {
      ...
      .then(async docRef => {
        ...
        for (let i = 0; i < images.length; i++) {
          try {
            ...
            let UploadedFile = await firebase.storage.uploadFile(
              path,
              images[i],
              null,
              options
    ...
    try {
      console.log("HEEEEEEEEEEREREREREREE£E^éreee");
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        let UploadedFile = await admin.uploadFile(
          path,
          images[i],
          null,
          options
