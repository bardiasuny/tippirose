...
export const orderProductMakeVip = (
  data,
  product,
  color,
  size,
  ...
  try {
    ...
    if (user.uid) {
      ...
      const secretKey = parseInt(Math.random() * 100000, 10);
      ...
      toastr.error(
        "OPSSS",
        "you should signed in before you can order a product "
      );
    }
  } catch (err) {
    console.log(err);
