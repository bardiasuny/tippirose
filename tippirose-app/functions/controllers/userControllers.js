...
exports.postActivateProduct = async (req, res, next) => {
  ...
  const secretKeyQuery = await admin
    .firestore()
    .collection("orders")
    .doc(product.orderId)
    .collection("secretKey")
  ...
  if (parseInt(userData.secretKey, 10) === secretKey.secretKey) {
    console.log("Hewrer");
    await firebase
      .firestore()
      .collection("users")
    ...
    admin
      .auth()
      .setCustomUserClaims(userId, { vip: true })
      .then(() => {
...
exports.postGetUserTemplate = async (req, res, next) => {
  ...
  const userTemplateQuery = await firebase
    .firestore()
    .collection("users")
    .doc(userId)
